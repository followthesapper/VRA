#!/usr/bin/env python3
"""
T6-D4 — Protein Normal Mode Detection: Sample Complexity
Fast FFT-GLRT (carrier-cancel + per-base AR(1) prewhitening + averaged PSD)

Pipeline
1) PM-only signal on a modular carrier.
2) Exact carrier-phase cancellation (we know N and each base a).
3) Residual phase unwrap + linear detrend (keep slow drift).
4) Per-base AR(1) phi estimate; prewhiten each base independently.
5) Hann-windowed rFFT per base → periodograms P_i(f); average across bases.
6) GLRT over a chosen band (default=full band) using
   Bonferroni-corrected p-value from the Gamma(k=M, θ=1/M) null for the
   averaged PSD (robust local-median noise floor).

Success = (reject H0 at overall α=δ) AND (|ω̂−ω| < ε).

Run:
  python3 T6D4_protein_modes.py
  # optional:
  #   --alpha 0.05   (overall significance; defaults to δ)
  #   --band 0.02    (half-width around ω_true; omit for full-band blind search)
  #   --trials 25    (faster quick pass)
"""
...
from scipy.special import gammaincc  # Gamma tail for averaged-PSD GLRT
...
def parse_args():
    p = argparse.ArgumentParser(description="T6-D4 Fast FFT-GLRT (averaged PSD; no knob babysitting)")
...
class Config:
    ...
    # Trajectory sizes
    L_values: Tuple[int, ...] = (2**8, 2**10, 2**12, 2**14, 2**15)

    # Monte Carlo
    n_trials: int = 50

    # Fixed signal defaults (no fine-tuning needed)
    mode_amplitude: float = 0.20     # PM depth (radians)
    sigma_thermal: float = 0.50      # phase noise (radians)
    phi_drift: float = 0.992         # slow AR(1) drift
    amp_jitter_std: float = 0.02     # tiny amplitude jitter (not coherent AM)
    ...
    def __post_init__(self):
        ...
        self.log_file = self.output_dir / f"T6D4_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        logging.basicConfig(
            level=logging.INFO,
        ...
        logging.info("="*70)
        logging.info("T6-D4: Fast FFT-GLRT (carrier-cancel + AR(1) prewhitening, averaged PSD)")
        logging.info("="*70)
...
# ================================================================
# SIGNAL GENERATION (PM-only) — returns sequences and bases used
# ================================================================
...
# ================================================================
# FAST FFT-GLRT (averaged PSD across bases)
# ================================================================
...
def fft_glrt_avg_psd_padj(residuals: List[np.ndarray],
    ...
    Non-coherent combine across bases:
      ...
      - under H0: P̄ bin ~ Gamma(k=M, θ=1/M).  p_bin = Q(k, k*T) = gammaincc(k, k*T)
    ...
    for y in residuals:
        ...
        if freqs is None or len(f) > len(freqs):
            freqs = f  # use longest grid as reference

    if not P_list or freqs is None:
        return False, float('inf'), 1.0

    # Align lengths across bases (trim to shortest among computed)
    Luse = min(len(P) for P in P_list)
    P_stack = np.stack([P[:Luse] for P in P_list], axis=0)
    f_use = freqs[:Luse]

    # Average PSD across bases
    ...
    # Robust noise = median of lower 80% of bins in the band
    ...
    # GLRT p-value: averaged exponential ⇒ Gamma(k=M, θ=1/M)
    T = float(P_avg[k_abs] / noise)
    kshape = max(1, M)  # integer shape
    p_bin = float(gammaincc(kshape, kshape * max(T, 0.0)))

    # Bonferroni over searched bins
...
def run_experiment(cfg: Config, alpha: float, band_halfwidth: float | None) -> List[Dict]:
    logging.info("\nStarting sample complexity experiment (Fast FFT-GLRT, averaged PSD)…")
    ...
    logging.info(f"Detector: FFT-GLRT (averaged PSD); α={alpha:.3f}; band={'full' if band_halfwidth is None else f'±{band_halfwidth}'}")
    ...
    for L in cfg.L_values:
        ...
        for eps in cfg.epsilon_values:
            ...
            for t in range(cfg.n_trials):
                ...
                # Residual per base (carrier-cancel → unwrap → linear detrend)
                residuals = []
                for i, a in enumerate(bases):
                    base_phase = reconstruct_base_phase(a, N, L)
                    z = seq[i] * np.exp(-1j * base_phase)
                    theta = np.unwrap(np.angle(z))
                    tvec = np.arange(L, dtype=float)
                    A = np.column_stack([tvec, np.ones(L)])
                    slope, intercept = np.linalg.lstsq(A, theta, rcond=None)[0]
                    theta_detr = theta - (slope*tvec + intercept)
                    residuals.append(theta_detr.astype(float))
...
def analyze_scaling(results: List[Dict], cfg: Config) -> Dict:
    ...
    if abs(slope + 2) < 0.5:
        logging.info("PASS — scaling matches ε⁻² within tolerance")
    elif slope < -1.0:
        logging.info("PARTIAL — sub-quadratic trend")
    else:
...
def plot_results(results: List[Dict], fit: Dict, cfg: Config):
    ...
    # Scaling
    ax[1].set_title("Sample Complexity Scaling (FFT-GLRT, averaged PSD)")
    ...
    if 'min_L_data' in fit and len(fit['min_L_data']) >= 3:
        ...
        ax[1].loglog(eps, Ls, 'o', label='Observed')
        er = np.logspace(np.log10(eps.min()), np.log10(eps.max()), 100)
        Lfit = np.exp(fit['slope']*np.log(er) + fit['intercept'])
        ax[1].loglog(er, Lfit, '--', label=f"Fit ε^{fit['slope']:.2f}")
        ax[1].loglog(er, np.exp(fit['intercept'])*er**(-2), ':', label='Theory ε⁻²')
        ax[1].legend()
    ax[1].grid(which='both', alpha=.3)

    fig.suptitle("T6-D4: Fast FFT-GLRT (carrier-cancel + AR(1), averaged PSD)", fontsize=14)
    out = cfg.figure_dir / "T6D4_protein_modes.png"
...
def main():
    ...
    with open(cfg.output_dir/'T6D4_results.json','w') as f:
