...
class Config:
    ...
    def __init__(self):
        ...
        self.log_file = self.output_dir / f'T6D2_v5_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
        self.setup_logging()
    ...
    def setup_logging(self):
        ...
        logging.info("="*70)
        logging.info("T6-D2 v5: Phonon Mode Separation - INFORMATIVE REGIME")
        logging.info("="*70)
...
def find_resolution_threshold(
    ...
    """
    Binary search to find Δω*(L) where P(resolve) ≈ 50%.
    ...
    logging.info(f"    Search range: [{delta_omega_low:.2e}, {delta_omega_high:.2e}]")
    ...
    for iteration in range(max_iterations):
        ...
        for trial in range(trials_per_test):
            ...
            sequences, metadata = generate_dual_phonon_signal_imperfect(
                ...
                seed=1000 + iteration * trials_per_test + trial
            )
        ...
        success_rate = successes / trials_per_test
        mean_delta_aic = np.mean(delta_aic_list)
...
def run_experiment():
    ...
    for L in config.L_values:
        ...
        delta_omega_star, history = find_resolution_threshold(
            ...
            config.omega_base,
            config.delta_omega_search_range,
    ...
    # Save results
    results_file = config.output_dir / 'T6D2_v5_results.json'
...
def analyze_scaling(results: Dict, config: Config):
    """Analyze scaling law Δω*(L) ∝ L^(-α)"""
    logging.info("="*70)
    logging.info("SCALING LAW ANALYSIS: Δω*(L) ∝ L^(-α)")
    logging.info("="*70)
    logging.info("")

    # Extract valid thresholds
    L_vals = []
    delta_omega_vals = []
    ...
    if len(L_vals) < 2:
        logging.info("VERDICT: INCONCLUSIVE (insufficient thresholds)")
        logging.info("")
        return
    ...
    logging.info(f"  R² = {r_value**2:.4f}, p = {p_value:.2e}")
    logging.info("")
    ...
    # Verdict
    if r_value**2 < 0.7:
        verdict = "INCONCLUSIVE (poor fit quality)"
    elif alpha < 0.65:
        verdict = "✓ PASS: Super-resolution scaling (α < 0.65)"
    elif alpha < 0.85:
        verdict = "MARGINAL: Between super-resolution and Rayleigh"
    else:
        verdict = "✗ FAIL: Classical Rayleigh scaling (α ≥ 0.85)"
...
def plot_results(results: Dict, config: Config):
    ...
    figure_path = config.figure_dir / 'T6D2_v5_resolution_scaling.png'
